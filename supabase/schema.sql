...
-- ============================================================================
-- activity_log: a meaningful-actions audit trail (not raw clicks) --
-- contact added/edited/deleted, a call logged, the Email link clicked.
-- Anyone logs their own actions as they happen; only a coordinator can read
-- it back (the admin-side "what did the caller do" dashboard, and later
-- the source data for the daily digest email). contact_label is a snapshot
-- of the contact's name at the time, not a live join, so the log still
-- reads correctly after a contact is deleted. Append-only: no update or
-- delete policy at all, on purpose.
-- ============================================================================
